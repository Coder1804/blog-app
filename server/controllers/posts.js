...
const getPost = (req,res)=>{
    const q = "SELECT p.id , `username` , `title`, `des` , p.img ,  u.img AS userImg , `cat` , p.date  FROM users u  JOIN posts p  ON  u.id = p.uid WHERE p.id=?"
    ...
    db.query(q,[req.params.id] , (err,data)=>{
        ...
        if(data.length === 0 ) return res.status(404).json("Bunday foydalanuchi yo'q")
...
const deletePost = (req,res)=>{
    ...
    jwt.verify(token,"jwtsecretkey" , {},(err,userInfo)=>{
        ...
        db.query(q , [postId,userInfo.id] , (err,data)=>{
            ...
            return res.json("Post hasbeen deleted!")
        })
    })
}
